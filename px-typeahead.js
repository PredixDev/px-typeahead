...
import './css/px-typeahead-styles.js';
...
Polymer({
  _template: html`
    ...
    <template is="dom-if" if="{{prefetchUrl}}">
      <iron-ajax auto="" url="{{prefetchUrl}}" headers="{{prefetchHeaders}}" handle-as="json" last-response="{{_prefetchedCandidates}}">
      </iron-ajax>
    </template>
    ...
    <div class="search__form">
      ...
      <template is="dom-if" if="[[_computeHasInputAndNotDisabled(inputValue, disabled)]]">
...
`,
  ...
  properties: {
    ...
    */
    ...
    /**
     * Headers for remote url searches
     */
    remoteUrlSearchHeaders: {
      type: Object,
      value: function() {
        return {}
      }
  ...
  _computeHasInputAndNotDisabled: function() {
    return this.inputValue.length && this.disabled === false;
  },

  _setSearchResultSize: function () {
    if (this.$$('#searchResults')) {
      var searchInputRect = this.$$('.search__box').getBoundingClientRect();
   ...
   */
  _onhover: function (e) {
    var alreadySelected = dom(this.root).querySelector('.item-selected');
    if (alreadySelected) {
  ...
  _onblur: function (e) {
    ...
    this.toggleClass('validation-error', this.required && !this.value, this.$$('.search__box'));
  },
  ...
  _downArrowPress: function (e) {
    var alreadySelected = dom(this.root).querySelector('.item-selected'),
      suggestionsMenu = dom(this.root).querySelector('ul'),
      selectedItem,
      children;
  ...
  _enterPress: function (e) {
    ...
    alreadySelected = dom(this.root).querySelector('.item-selected');

    //if we only have 1 option, select it, otherwise, do nothing.
    results = dom(this.root).querySelectorAll('.result');
    if (results.length === 1) {
    ...
    this.toggleClass('validation-error', false, this.$$('.search__box'));
    this.fire('px-typeahead-item-selected', this.value);
    dom(this.root).querySelector('input').focus();
  },
  ...
  _checkIfSingleResult: function (e) {
    var results = dom(this.root).querySelectorAll('.result');
  ...
  _remoteUrlSearch: function (term) {
    ...
    ironAjaxEl.headers = this.remoteUrlSearchHeaders;
    ironAjaxEl.url = url;
